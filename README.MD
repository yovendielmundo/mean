...
### Docker
```
$ docker build -t souqalmal-test .
```
```
$ docker pull mongo
$ docker run -d --name db mongo[ --noprealloc][ --smallfiles][ --nojournal]
$ docker run -p 3000:3000 --link db:db souqalmal-test
```
or easy way
```
$ docker-compose up
...
```
$ docker exec -it `docker ps -q -l` gulp test
```
